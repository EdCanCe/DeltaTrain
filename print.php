...
if(isset($_SESSION["CurrentUserIDSession"])){ #Checa si ya inició sesión
    ...
    echo "<script> window. location='../login'</script>"; #Como no iniciaste sesión te manda a hacerlo
}
...
$pdf->Cell(80,10,utf8_decode('Descripción'),1,1,'C');
...
while($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell(50,7,$row['User_Changes'],1,0,'C');
    $pdf->Cell(20,7,$row['Table_Changes'],1,0,'C');
    $pdf->Cell(30,7,$row['Time_Changes'],1,0,'C');
    $pdf->Cell(80,7,utf8_decode($row['Description_Changes']),1,1,'C');
}
